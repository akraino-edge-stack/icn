...
function test_addons {
    ...
    if [[ "${container_runtime}" == "containerd://1.2.13" ]]; then
        ...
        kud_tests="topology-manager-sriov: multus:multus-cni ovn4nfv:ovn4nfv-network nfd:node-feature-discovery sriov-network:sriov-network cmk:cpu-manager"
    else
        kud_tests="topology-manager-sriov: multus:multus-cni ovn4nfv:ovn4nfv-network nfd:node-feature-discovery sriov-network:sriov-network qat:qat-device-plugin cmk:cpu-manager"
    fi
