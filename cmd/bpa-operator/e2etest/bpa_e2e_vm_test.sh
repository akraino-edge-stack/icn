...
if !(wget https://github.com/containernetworking/plugins/releases/download/v0.8.2/$CNI_PLUGINS -P $TUNING_DIR); then
...
do
	printf "\nDescribe net-attach-def....\n"
	val=$(kubectl describe net-attach-def)
	printf "\n$val\n"

	printf "\nDescribe virtlet-deployment pod....\n"
	val=$(kubectl describe pods $(kubectl get pods |grep -i virtlet-deployment | awk '{print $1}'))
	printf "\n$val\n"
...
podjson=$(kubectl get pods $(kubectl get pods |grep -i test-vm | awk '{print $1}') -o json)
printf "\n$podjson\n\n"
...
kubectl exec -it $kudjob -- cat /root/.ssh/id_rsa
kubectl exec -it $kudjob -- cat /root/.ssh/id_rsa.pub
...
kubectl delete -f e2e_vm_test_provisioning_cr.yaml
