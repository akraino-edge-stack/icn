...
function install_jenkins {
    ...
    pip3 install ansible jenkins-job-builder
    ...
    if [[ ${WITH_VAGRANT} == "yes" ]]; then
        ...
        apt-get update
        apt-get install -y libvirt-bin libvirt-dev vagrant xmlstarlet
        usermod -aG libvirt jenkins
        usermod -aG kvm jenkins
        # Vagrant plugins must be installed per-user
        sudo -H -u jenkins vagrant plugin install vagrant-libvirt
    fi
...
function install_jenkins_id {
    ...
    ssh-keygen -q -t rsa -N '' -f /var/lib/jenkins/jenkins-rsa <<<n >/dev/null 2>&1 || true
    ssh-copy-id -i /var/lib/jenkins/jenkins-rsa ${CLUSTER_SSH_USER}@${CLUSTER_MASTER_IP}
